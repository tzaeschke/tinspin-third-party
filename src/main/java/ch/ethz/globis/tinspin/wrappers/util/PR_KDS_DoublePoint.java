 ...
 */
public class PR_KDS_DoublePoint
implements Point<Double>
{
	 ...
	 * greater than 0.
	 */
	public PR_KDS_DoublePoint(int dimensions) {
		assert(dimensions > 0);
	 ...
	 */
	public PR_KDS_DoublePoint(double x, double y) {
		this(2);
		setCoord(0, x);
		setCoord(1, y);
	}
	 ...
	 */
	public PR_KDS_DoublePoint(double x, double y, double z) {
		this(3);
		setCoord(0, x);
		setCoord(1, y);
		setCoord(2, z);
	 ...
	 */
	public PR_KDS_DoublePoint(double x, double y, double z, double w) {
		this(4);
		setCoord(0, x);
		setCoord(1, y);
		setCoord(2, z);
	...
	public Double getCoord(int dimension) {
		return Double.valueOf(__coordinates[dimension]);
	}
	...
	public boolean equals(Object obj) {
		if(!(obj instanceof PR_KDS_DoublePoint)) 
			return false;

		PR_KDS_DoublePoint point = (PR_KDS_DoublePoint)obj;

		for(int i = 0; i < __coordinates.length; ++i)
			//if(!point.getCoord(i).equals(__coordinates[i]))
			if(__coordinates[i] != point.__coordinates[i])
				return false;
	...
	@Override
	public Object clone() {
		PR_KDS_DoublePoint point =
				new PR_KDS_DoublePoint(__coordinates.length);
		for(int i = 0; i < __coordinates.length; ++i)
			point.setCoord(i, __coordinates[i]);
		return point;
	}
